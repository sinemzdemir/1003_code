...
def select_best_model_with_hyperparameter_tuning(representation_name,integrated_lst,models=["RandomForestClassifier",'SVC',"KNeighborsClassifier",'Fully Connected Neural Network'],auto=True,total_run_count=1):
    ...
    for classifier in models:            
        ...
        if(classifier== "Fully Connected Neural Network"):
            ...
            model_count=model_count+1                                
            model_tunning = GridSearchCV(estimator=model_pipline, param_grid=parameters, cv=kf,pre_dispatch = 20,scoring='f1_weighted', n_jobs=-1)            
        ...
        if (classifier== "Fully Connected Neural Network"):
            ...
            label_lst_test=[]
            model_label_pred_lst=[]
            model,model_label_pred_lst,label_lst,protein_name_tr,parameter,protein_name,parameter,model_label_pred_test_lst,label_lst_test = NN(kf,protein_representation,model_label,input_size,representation_name,protein_and_representation_dictionary)        
            #model,model_label_pred_lst,label_lst,protein_name_tr,protein_name,parameter,model_label_pred_test_lst,label_lst_test
            neural_network_eval(kf,model,protein_representation,model_label_pred_lst,label_lst,index,representation_name,classifier_name,protein_and_representation_dictionary,file_name,"training",protein_name_tr,classifier_name_lst,path,auto,parameter)
            
            neural_network_eval(kf,model,protein_representation,model_label_pred_test_lst,label_lst_test,index,representation_name,classifier_name,protein_and_representation_dictionary,file_name,"test",protein_name,classifier_name_lst,path,auto,parameter)
            ...
            rep_name='_'.join(representation_name)
            binary_evaluate.evaluate(kf, protein_representation, model_label_pred_lst, label_lst, classifier_name,rep_name,protein_and_representation_dictionary,file_name,index,"training",class_len)
            col_names=["Label"]
            label_predictions=pd.DataFrame(np.concatenate(model_label_pred_lst),columns=col_names)            
            label_predictions.insert(0, "protein_id", protein_name)                  
            label_predictions.to_csv(path+'/'+'training'+'/'+rep_name +'_'+"binary_classifier"+ '_' + classifier_name +'_training'+ "_predictions5cv.tsv",sep="\t", index=False)        
        class_name='_'.join(classifier_name_lst)
        best_parameter_df.to_csv(path+'/'+'training'+'/'+class_name+'_'+rep_name+'_'+"binary_classifier"+'_best_parameter' +'.csv',index=False)
    if auto==True:
