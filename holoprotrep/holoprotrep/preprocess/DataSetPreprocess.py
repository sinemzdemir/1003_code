...
def integrate_go_lables_and_representations_for_multilabel(
    label_dataframe, dataset_name, n
):
    """
    ...
    for index, row in tqdm.tqdm(vals.iterrows(), total=len(vals)):
        ...
        representation_dataset.loc[index] = [dataset_name.iloc[index]["Entry"]] + [
            list_of_floats
        ]
    if n == 3:
        for index in range(len(aspect_lst)):
            integrated_dataframe = label_dataframe[index].merge(
        ...
        concate_columns = list(
            set(integrated_dataframe.columns) - set({"Entry", "Vector"})
        )
        integrated_dataframe["Label"] = [
            j
            for i in integrated_dataframe.apply(
