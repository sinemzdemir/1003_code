...
if "prepare_datasets" in choice_of_task_name:
    if "fuse_representations" in choice_of_task_name:
        positive_sample_dataframe = pd.read_csv(
            data["parameters"]["prepare_datasets"]["positive_sample_data"][0]
        )
        negative_sample_dataframe = pd.read_csv(
            data["parameters"]["prepare_datasets"]["negative_sample_data"][0]
        )
        ...
        total_sample_dataframe = negative_sample_dataframe.append(
        ...
        total_sample_dataframe = shuffle(negative_sample_dataframe,random_state=random_state)
        
        datapreprocessed_lst.append(
            Binary_DataSetPreprocess.integrate_go_lables_and_representations_for_binary(
                total_sample_dataframe, representation_dataframe, 1
            )
        )
...
if "model_training" in choice_of_task_name:
    ...
    if "prepare_datasets" in choice_of_task_name:
        ...
        for data_preproceed in preprocesed_data_path:
            ...
            best_param = BinaryTrainModelsWithHyperParameterOptimization.select_best_model_with_hyperparameter_tuning(
                data["parameters"]["model_training"]["representation_names"],
                data_preproceed_df,
                scoring_func,
                data["parameters"]["model_training"]["classifier_name"],
            )
...
if "prediction" in choice_of_task_name:
    ...
    for i in data["parameters"]["prediction"]["prepared_path"]:
        ...
        binary_prediction.make_prediction(
            data["parameters"]["prediction"]["representation_names"][0],
            test_data,
            data["parameters"]["prediction"]["model_directory"],
            classifier_name_lst,
        )
