...
if "prepare_datasets" in choice_of_task_name:
    if "fuse_representations" in choice_of_task_name :
        for i in range(len(data["parameters"]["prepare_datasets"]["annotation_files"])):
            list_of_go_ids_and_annotations_dataframe=pd.read_csv(data["parameters"]["prepare_datasets"]["annotation_files"][i])
...
if "model_training" in choice_of_task_name:
    if "prepare_datasets" in choice_of_task_name:
        ...
        for data_preproceed in glob.glob(preprocesed_data_directory_path[0]+"*.pickle"):
            ...
            best_param=TrainModelsWithHyperParameterOptimization.select_best_model_with_hyperparameter_tuning(data["parameters"]["model_training"]["representation_names"],data_preproceed.split('.')[0].split("/")[-1],data_preproceed_df,data["parameters"]["model_training"]["classifier_name"],data["parameters"]["model_training"]["auto"])   
            if "model_test" in choice_of_task_name:
                Test_score_calculator.Model_test(representation_names,data_preproceed,data["parameters"]["model_test"]["best_parameter_file"])
if "model_test" in choice_of_task_name:
    if len(choice_of_task_name)==1:
        Test_score_calculator.Model_test(data_preproceed,data["parameters"]["model_test"]["representation_names"],data["parameters"]["model_test"]["prepared_directory_path"],data["parameters"]["model_test"]["best_parameter_file"])
