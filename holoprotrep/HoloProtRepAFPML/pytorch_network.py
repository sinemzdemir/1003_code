...
def NN(kf,protein_representation,model_label, input_size,representation_name,protein_and_representation_dictionary):
    ...
    for fold_train_index, fold_test_index in kf.split(protein_representation, model_label):
        ...
        x=x_df['Vector']
        x_test_df=pd.DataFrame(protein_representation['Vector'],index=list(fold_test_index))
        ...
        for epoch in range(900):
            ...
            with torch.no_grad():
                ...
                loss_val  = criterion(out_probs, y_test)    
        ...
        for t in range(1, 101):
            threshold = t / 100.0
            fscore=TrainModelsWithHyperParameterOptimization.evaluate_annotation_f_max( model_label[fold_test_index],y_test )
            if fmax < fscore:
                fmax = fscore
                tmax = threshold
        ...
        for t in range(1, 101):
            threshold = t / 100.0
            fscore=TrainModelsWithHyperParameterOptimization.evaluate_annotation_f_max( model_label[fold_train_index],y )
            if fmax < fscore:
                fmax = fscore
                tmax = threshold
