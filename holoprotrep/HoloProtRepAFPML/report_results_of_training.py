...
def report_results_of_training(representation_name,result_list,mean_result_list,classifier_name,file_name,index,classifier_type):
    ...
    result_len=27 
    if index==1:
        result_df = pd.DataFrame(columns=["representation_name","classifier_name", "accuracy", "std_accuracy",  "f1_micro", "std_f1_micro", "f1_macro", "std_f1_macro","f_max", "std_f_max", "f1_weighted","std_f1_weighted","precision_micro", "std_precision_micro","precision_macro", "std_precision_macro", "precision_weighted", "std_precision_weighted", "recall_micro", "std_recall_micro","recall_macro", "std_recall_macro", "recall_weighted", "std_recall_weighted",  "hamming distance","std_hamming distance","auc","std_auc","matthews correlation coefficient"])           
        result_df.loc[index-1] = result_list[0]
        result_df.reset_index(drop=True, inplace=True)                
        mean_result_dataframe = pd.DataFrame(columns=["representation_name","classifier_name", "accuracy", "std_accuracy",  "f1_micro", "std_f1_micro", "f1_macro", "std_f1_macro","f_max", "std_f_max", "f1_weighted","std_f1_weighted","precision_micro", "std_precision_micro","precision_macro", "std_precision_macro", "precision_weighted", "std_precision_weighted", "recall_micro", "std_recall_micro","recall_macro", "std_recall_macro", "recall_weighted", "std_recall_weighted",  "hamming distance","std_hamming distance","auc","std_auc","matthews correlation coefficient"])
    ...
    #rep_name="_".join(representation_name)
        ...
        if(index>2):
            result_df=pd.read_csv(path+'/'+classifier_type+'/'+representation_name+'_'+file_name+'_'+classifier_type+".tsv", sep="\t")
            result_df.loc[index-1] = result_list[0]
            result_df.reset_index(drop=True, inplace=True)
            result_df.to_csv(path+'/'+classifier_type+'/'+representation_name+'_'+file_name+'_'+classifier_type+".tsv", sep="\t", index=False)
