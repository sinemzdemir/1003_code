...
def evaluate(kf, protein_representation, model_label_pred_lst, label_lst,f_max_cv, classifier_name,representation_name,protein_and_representation_dictionary,file_name,index_,eval_type,mlt="multilabel"):
    ...
    for fold_index in range(len(model_label_pred_lst)):
        if mlt=="binary":
            mcm = confusion_matrix(label_lst[fold_index], model_label_pred_lst[fold_index])
        
        else:       
            mcm = multilabel_confusion_matrix(label_lst[fold_index], model_label_pred_lst[fold_index])
        ...
        for result_element in range(len(mcm)):
            acc.append((mcm[result_element][0][0] + mcm[result_element][1][1]) / (
                            mcm[result_element][0][0] + mcm[result_element][0][1] + mcm[result_element][1][0] +
                            mcm[result_element][1][1]))
        mc.append(((sum(tp) * sum(tn)) - (sum(fp) * sum(fn))) / math.sqrt(
                (sum(tp) + sum(fp)) * (sum(tp) + sum(fn)) * (sum(tn) + sum(fp)) * (sum(tn) + sum(fn))))
        mcc_cv.append(mc)
        ac = np.mean(acc)
        acc_cv.append(ac)
